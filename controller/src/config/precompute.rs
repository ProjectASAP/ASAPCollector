...
fn build_query_expr(w: &QueryWorkload) -> String {
    ...
    let filters: Vec<String> = w.label_filters.iter()
        .map(|(k, v)| format!("{k}=\"{v}\""))
        .collect();
    ...
    format!("{agg_fn}(0.99, {selector}[{}])", format_duration(w.time_window))
}

fn build_store_path(w: &QueryWorkload) -> String {
    format!("precomputed/{}/p99/{}", w.metric_name, format_duration(w.time_window))
}
...
impl PrecomputeClient {
    ...
    pub async fn register(&self, job: &PrecomputeJob) -> anyhow::Result<JobResponse> {
        ...
        resp.json::<JobResponse>().await.context("decode job response")
    }
...
mod tests {
    ...
    use axum::{Router, routing::{post, delete}, Json, http::StatusCode};
    use serde_json::json;
    ...
    fn should_precompute_true_when_repeat_lt_latency() {
        assert!(should_precompute(&w(Some(Duration::from_secs(60)), Some(Duration::from_secs(300)))));
    }

    #[test]
    fn should_precompute_false_when_repeat_geq_latency() {
        assert!(!should_precompute(&w(Some(Duration::from_secs(300)), Some(Duration::from_secs(60)))));
    }
    ...
    fn build_jobs_returns_job_when_eligible() {
        let workload = w(Some(Duration::from_secs(60)), Some(Duration::from_secs(600)));
    ...
    fn build_jobs_empty_when_not_eligible() {
        let workload = w(Some(Duration::from_secs(300)), Some(Duration::from_secs(60)));
        let jobs = build_precompute_jobs(&workload, &dummy_plan(), "backend:4317");
    ...
    fn cardinality_uses_count_distinct_expr() {
        let mut workload = w(Some(Duration::from_secs(60)), Some(Duration::from_secs(600)));
        workload.aggregations = vec![AggType::Cardinality];
        let jobs = build_precompute_jobs(&workload, &dummy_plan(), "backend:4317");
        assert!(jobs[0].query_expr.contains("count_distinct_over_time"),
            "got: {}", jobs[0].query_expr);
    }

    #[tokio::test]
    async fn client_register_success() {
        let app = Router::new().route("/api/v1/precompute/jobs", post(|| async {
        ...
        let resp = client.register(&PrecomputeJob {
    ...
    async fn client_register_error_on_bad_status() {
        let app = Router::new().route("/api/v1/precompute/jobs",
        ...
        let client = PrecomputeClient::new(format!("http://{addr}"));
        assert!(client.register(&PrecomputeJob {
            query_expr: "q".into(), granularity: Duration::from_secs(60),
            sketch_source: "s".into(), store_path: "p".into(),
        }).await.is_err());
    }

    #[tokio::test]
    async fn client_deregister_success() {
        let app = Router::new().route("/api/v1/precompute/jobs/:id",
