...
func (p *gorillaProcessor) uploadWithRetry(key string, data []byte) error {
...
func (p *gorillaProcessor) uploadOnce(ctx context.Context, key string, data []byte) error {
...
func (p *gorillaProcessor) multipartUpload(ctx context.Context, key string, data []byte) error {
...
func writeLocalFile(localDir string, key string, data []byte) (string, error) {
	...
	if err := os.MkdirAll(filepath.Dir(localPath), 0o755); err != nil {
		return "", fmt.Errorf("gorilla: create local path: %w", err)
	}
	tmp := localPath + ".tmp"
	if err := os.WriteFile(tmp, data, 0o644); err != nil {
		return "", fmt.Errorf("gorilla: write temp file: %w", err)
	}
	if err := os.Rename(tmp, localPath); err != nil {
		return "", fmt.Errorf("gorilla: rename temp file: %w", err)
	}
	return localPath, nil
}

// buildObjectKey generates the S3 key for a compressed block.
func buildObjectKey(prefix string, fixedName string, blockEnd time.Time, idx int) string {
	...
	base := fmt.Sprintf("block-%d-%02d-%08x.gorilla", blockEnd.Unix(), idx, rand.Uint32())
	if pfx != "" {
