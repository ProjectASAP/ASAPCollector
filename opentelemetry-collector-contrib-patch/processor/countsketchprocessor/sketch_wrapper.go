...
// interfaces the asap-precompute-go runtime requires (Phase 2 step 2.8).
//
...
func newCountSketchWrapper(rows, cols int) (*countSketchWrapper, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("countsketchprocessor: NewCountSketch(%d, %d): %w", rows, cols, err)
	}
	return &countSketchWrapper{cs: cs, rows: rows, cols: cols}, nil
}

// updateString mirrors the legacy ws.cs.UpdateString(itemKey, value)
// call. The shim's observer routes KindFloat observations through
// this method so the runtime drives the same hot path the legacy
// processor used.
...
func (w *countSketchWrapper) ApplyDelta(delta []byte) error {
	if len(delta) == 0 {
		return errors.New("countsketchprocessor wrapper: ApplyDelta with empty payload")
	}
	if w.cs == nil {
	...
	if err != nil {
		return fmt.Errorf("countsketchprocessor wrapper: DeserializeDelta: %w", err)
	}
...
// CountSketch via UpdateString. Legacy hot path: ws.cs.UpdateString(
// metric.Name(), value). The metric name travels through the host-
// neutral interface as ObservationValue.Bytes (set by the shim's
// observe path); when absent, the observer falls back to the runtime
// config's MetricName.
...
// Compile-time assertions that the wrapper satisfies both the base
// Sketch trait (used by the runtime's window state machine) and the
// FrequencySketch query trait (used by adapter code that needs typed
// frequency queries).
var (
