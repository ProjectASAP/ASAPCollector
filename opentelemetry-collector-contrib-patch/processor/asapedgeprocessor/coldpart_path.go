...
func fragmentsToSeries(frags []gorilla.Fragment, external map[string]string) ([]coldpart.Series, int64, int64, error) {
	...
	for i := range frags {
		...
		if err != nil {
			return nil, 0, 0, fmt.Errorf("coldpart: decode fragment %d: %w", i, err)
		}
		if len(samples) == 0 {
			continue
		}
		ls := gorilla.FragmentLabels(f.MetricName, f.Attributes, external)
		key := ls.String()
		a := byKey[key]
		if a == nil {
			a = &acc{lbls: ls}
		...
		for _, sm := range samples {
			if !haveBounds {
				blockStart, blockEnd, haveBounds = sm.T, sm.T, true
				continue
			}
			if sm.T < blockStart {
				blockStart = sm.T
	...
	out := make([]coldpart.Series, 0, len(order))
	for _, key := range order {
		a := byKey[key]
		// A series may span multiple fragments (separate XOR chunks); ensure the
		// merged run is time-ordered ascending, as coldpart requires.
		sort.SliceStable(a.samples, func(i, j int) bool { return a.samples[i].T < a.samples[j].T })
		out = append(out, coldpart.Series{Labels: a.lbls, Samples: a.samples})
	}
	return out, blockStart, blockEnd, nil
}
