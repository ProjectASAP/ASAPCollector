...
type Config struct {
	...
	// ASAP patch — RemoteConfigPath enables apply-on-the-fly for
	// `AgentRemoteConfig` messages pushed by the OpAMP server. When set
	// to a non-empty path, the extension's `onMessage` handler:
	//   1. picks the first ConfigFile body from the message (preferring
	//      the entries named "" or "asap-otel" by convention),
	//   2. writes it atomically to this path,
	//   3. reports `RemoteConfigStatuses_APPLIED` to the server,
	//   4. exits the collector process with code 0 — Docker's
	//      `restart=unless-stopped` (or systemd `Restart=always`) is
	//      then expected to bring the container back up, loading the
	//      freshly-written config from disk.
	//
	// When empty (the default and the upstream-extension behavior),
	// incoming RemoteConfig messages are ignored and the extension
	// stays "report-only" (effective config UP, nothing DOWN).
	//
	// Production use in the ASAP MVP demo: set to the same path the
	// collector loads at boot (e.g. `/etc/otel/config.yaml` in the
	// asap-otel container image). The orchestration layer is
	// responsible for the restart loop.
