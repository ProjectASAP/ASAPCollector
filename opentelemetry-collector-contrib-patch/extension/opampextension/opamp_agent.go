...
func (o *opampAgent) Start(ctx context.Context, host component.Host) error {
	...
	// ASAP patch — when `remote_config_path` is set, advertise
	// `AcceptsRemoteConfig` so the OpAMP client library actually
	// surfaces incoming `ServerToAgent.remote_config` to OnMessage.
	// Without this capability the upstream client at
	// receivedprocessor.go:83 silently drops the message with
	// `"Ignoring RemoteConfig, agent does not have
...
func (o *opampAgent) onMessage(ctx context.Context, msg *types.MessageData) {
	...
	// ASAP patch — apply pushed RemoteConfig when `remote_config_path`
	// is configured. Without this hook the upstream extension is
	// report-only (effective config UP, nothing DOWN); the asap-otel
	// agent then ignores every controller-emitted config and stays
	// on its static bootstrap YAML — the central blocker tracked in
	// ASAPCollector#381.
	if msg.RemoteConfig != nil && o.cfg.RemoteConfigPath != "" {
		o.processRemoteConfig(ctx, msg.RemoteConfig)
	}
}

// processRemoteConfig applies an OpAMP AgentRemoteConfig push by
// writing the YAML body to disk and exiting the process. Docker
// restart (or systemd Restart=always) is expected to bring the
// collector back up with the new config.
//
// Status is reported synchronously to the OpAMP server: APPLIED on
// successful write, FAILED with the error string otherwise.
//
// Refer to RemoteConfigPath's doc on Config for the supervisor-style
// rationale + ASAP MVP use case.
func (o *opampAgent) processRemoteConfig(_ context.Context, rc *protobufs.AgentRemoteConfig) {
	if rc.GetConfig() == nil {
		o.logger.Warn("OpAMP RemoteConfig has no ConfigMap; ignoring")
		return
	}
	// Pick the first non-empty body, preferring the conventional
	// entries first ("" = single-file convention, "asap-otel" = ASAP
	// controller's named-entry convention). Fall back to any other
	// entry to stay robust against controller renamings.
	var (
		body []byte
		name string
	)
	cm := rc.GetConfig().GetConfigMap()
	for _, key := range []string{"", "asap-otel"} {
		if cf, ok := cm[key]; ok && len(cf.GetBody()) > 0 {
			body = cf.GetBody()
			name = key
			break
		}
	...
	if body == nil {
		o.logger.Warn("OpAMP RemoteConfig has no non-empty ConfigFile body; ignoring")
		return
	}

	// Cheap YAML well-formedness gate. Catches the most common
	// emitter bugs (controller serialization mishap, truncated push)
	// before we restart on a guaranteed-broken config.
	var probe map[string]any
	if err := yaml.Unmarshal(body, &probe); err != nil {
		o.logger.Error(
			"OpAMP RemoteConfig failed YAML parse; reporting FAILED, not restarting",
			zap.String("entry", name), zap.Int("bytes", len(body)), zap.Error(err))
	...
	// Direct write (truncate-and-replace). We deliberately don't use
	// the more conventional write-tmp + atomic-rename pattern: when
	// the target path is a Docker bind-mounted FILE, Linux's
	// `renameat2()` returns EBUSY because the inode is open by the
	// bind-mount machinery, even when the writer is the only
	// process holding the fd. Direct `WriteFile` truncates the
	// existing file in place and writes the new contents — which
	// IS supported on bind-mounted files. The torn-write window is
	// the duration of `WriteFile`'s syscall; since the collector
	// exits ~200ms later anyway, a partial-write race is bounded
	// by the orchestration layer's restart loop reading whatever
	// landed.
	dir := filepath.Dir(o.cfg.RemoteConfigPath)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		o.logger.Error("OpAMP RemoteConfig: cannot create parent dir",
			zap.String("dir", dir), zap.Error(err))
		_ = o.opampClient.SetRemoteConfigStatus(&protobufs.RemoteConfigStatus{
			LastRemoteConfigHash: rc.GetConfigHash(),
			Status:               protobufs.RemoteConfigStatuses_RemoteConfigStatuses_FAILED,
			ErrorMessage:         "mkdir " + dir + ": " + err.Error(),
		})
		return
	}
	if err := os.WriteFile(o.cfg.RemoteConfigPath, body, 0o644); err != nil {
		o.logger.Error("OpAMP RemoteConfig: write failed",
			zap.String("path", o.cfg.RemoteConfigPath), zap.Error(err))
		_ = o.opampClient.SetRemoteConfigStatus(&protobufs.RemoteConfigStatus{
			LastRemoteConfigHash: rc.GetConfigHash(),
			Status:               protobufs.RemoteConfigStatuses_RemoteConfigStatuses_FAILED,
			ErrorMessage:         "write " + o.cfg.RemoteConfigPath + ": " + err.Error(),
		})
		return
	}
	...
	o.logger.Info(
		"Applied OpAMP RemoteConfig; exiting collector process so the orchestration layer "+
			"restarts with the freshly-written config",
		zap.String("path", o.cfg.RemoteConfigPath),
		zap.String("entry", name),
		zap.Int("bytes", len(body)))

	// Brief delay so the SetRemoteConfigStatus message has a chance
	// to flush through the WebSocket before the process exits.
	time.Sleep(200 * time.Millisecond)
	// Exit code 0: clean shutdown. Docker `restart=unless-stopped` /
	// systemd `Restart=always` brings the collector back up.
	os.Exit(0)
}
