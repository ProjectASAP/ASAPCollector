...
Generate DDSketch-encoded metrics:

```console
./telemetrygen metrics \
  --workers 1 \
  --duration 5m \
  --metric-type DDSketch \
  --unique-timeseries \
  --unique-timeseries-duration 5m \
  --otlp-endpoint localhost:4317 \
  --otlp-insecure

# or 

go run . metrics --metric-type DDSketch --duration 5s --otlp-insecure
```

Setting --rate 0 disables throttling entirely.
Use --unique-timeseries together with --unique-timeseries-duration to control how many distinct metric series are produced within a window.
