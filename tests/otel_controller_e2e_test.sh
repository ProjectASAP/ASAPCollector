#!/usr/bin/env bash
# e2e_test.sh — End-to-end integration test: controller + asap-otel + e2esdkbench
#
# Usage:
...
#   - go             (Go toolchain)
#   - asap-otel      (built via ../../build_asap_otel.sh)
#   - curl, python3
...
if [[ ! -x "$ASAP_OTEL" ]]; then
  ...
  echo "       Run: ${ROOT}/build_asap_otel.sh" >&2
  exit 1
fi
...
echo "==> [Step 5] Starting asap-otel (OTLP :4317, Prom :8889)..."
"$ASAP_OTEL" \
