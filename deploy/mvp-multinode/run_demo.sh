...
#
# Drives the same six criteria as deploy/scripts/run_mvp_demo.sh
...
sync_to() {
    ...
    rsync -a "${ROOT}/deploy/scripts/" "${node}:/mydata/mvp-multinode/scripts/"
    rsync -a "${SCRIPT_DIR}/topology.env" "${node}:/mydata/mvp-multinode/topology.env"
}
