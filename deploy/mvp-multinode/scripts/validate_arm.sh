...
#   OTELAPP_FREQ_HZ         → expected events/series/sec
...
# Expected ≈ OTELAPP_FREQ_HZ × 10 (e.g. 100 Hz × 10 s = 1000 samples).
SAMPLE_BODY=$(q "count_over_time(${METRIC}[10s])")
EXPECTED_SAMPLES_10S=$((OTELAPP_FREQ_HZ * 10))
...
RATE_OK=$(python3 -c "
e=${OTELAPP_FREQ_HZ}; got=${PER_SERIES_RATE:-0}
...
| samples per series in last 10s | ~${OTELAPP_FREQ_HZ}×10 = ${EXPECTED_SAMPLES_10S} | min=${PER_SERIES_SAMPLES_MIN}, mean=${PER_SERIES_SAMPLES_MEAN}, max=${PER_SERIES_SAMPLES_MAX} | ${SAMPLES_OK} |
