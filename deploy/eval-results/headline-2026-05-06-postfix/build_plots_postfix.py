#!/usr/bin/env python3
"""Build the four §5 plots from the merged sweep + post-fix overlay.

Postfix-aware variant of headline-2026-05-06/build_plots.py: takes
an arbitrary list of sweep roots (`--sweep ROOT` repeatable). Cells
with the same name across roots resolve LAST-WINS, so the post-fix
subset overlays the original 60-cell data.

Inputs:
  --accuracy        merged accuracy.csv (concat of per-cell)
  --out-dir         output dir for PNGs + CSVs
  --sweep ROOT      sweep root; pass once per root, last wins per cell
...
def cell_dirs(roots):
    """Yield (cell_name, cell_dir) across all sweep roots, LAST WINS.

    Iterating roots in order then resolving a single dict-keyed
    by cell name lets the caller pass [old, new, postfix] and have
    postfix cells override the matching old/new cells without
    losing the cells that weren't re-run.
    """
    chosen = {}
    for root in roots:
        ...
        for ent in sorted(os.listdir(root)):
            ...
            if not os.path.isfile(os.path.join(full, "measurement.csv")):
                continue
            chosen[ent] = full
    for ent in sorted(chosen):
        yield ent, chosen[ent]
...
def main():
    ...
    ap.add_argument("--sweep", action="append", required=True,
                    help="sweep root; pass once per root, last wins per cell")
    args = ap.parse_args()
