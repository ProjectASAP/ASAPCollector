#!/usr/bin/env bash
# Reverse of restore_otel_proto_patches.sh.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC_PROTO="${ROOT_DIR}/opentelemetry-proto"
DEST_DIR="${ROOT_DIR}/opentelemetry-proto-patch"
...
ensure_git_repo() {
	...
	if ! git -C "${repo_dir}" -c safe.directory="${repo_dir}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
		echo "Directory ${repo_dir} is not a git repository (did you check out the proto source?)" >&2
		exit 1
	fi
}
...
backup_repo "${SRC_PROTO}" "${DEST_DIR}"
