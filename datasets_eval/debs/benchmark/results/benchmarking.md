...
| Non-statistical queries (Q2, Q9–Q12) | `throughput`, full day, `--batch-size 5000`, max speed |
...
> **Note:** DDSketch p0/p100 (min/max) are essentially exact on this data — the first window is 0.9997 (one symbol with a ≤2% boundary error at market open), every subsequent window is perfect.
...
> **Note:** HLL with precision 14 achieves near-perfect cardinality estimates on this data. Max relative error never exceeds 0.13% (vs 2% threshold).
...
Non-statistical queries (Q2, Q9–Q12) use a NOP collector — no sketch, no accuracy evaluation. Results are throughput and latency from `--mode throughput` full-day runs.
