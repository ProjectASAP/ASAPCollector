...
# Used to identify the richest Prometheus scrape snapshot for that query.
_SKETCH_METRIC_PATTERN: dict[str, str] = {
    "Q1": r"ddsketch|kll",
    "Q3": r"countsketch",
    "Q4": r"ddsketch|kll",
    "Q5": r"ddsketch|kll",
    "Q6": r"hll_cardinality",
    "Q7": r"ddsketch|kll",
    "Q8": r"ddsketch|kll",
}
...
def compare_q1(
    ...
    *,
    ...
    # Q1: skip first window (EMA cold-start), then compare sketch p50 vs EMA38
    # across ALL remaining windows (merge on symbol only, no window filter).
...
def compare_q3(
    ...
    *,
    ...
    if sk.empty:
        return {"metric": "q3_score", "value": 0.0, "threshold": 1.0, "pass": 0}
    top_sk = sk.nlargest(k, "est")
    overlap = len(set(top_gt["symbol"]) & set(top_sk["symbol"])) / float(k)
