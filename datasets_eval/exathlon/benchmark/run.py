...
from common import DEFAULT_FILES, file_tag_safe, METRIC_NAME, Q1_KLL_STREAMS
...
QUERY_CONFIG: dict[str, QueryCfg] = {
    ...
    "Q1_KLL": QueryCfg(  # Q1_KLL — dedicated KLL sketch accuracy benchmark
        aggregations=("p50", "p95", "p99"),  # Q1_KLL
        time_window="5m",  # Q1_KLL
        group_by=("entity", "metric_base"),  # Q1_KLL
        sketch_family="quantile",  # Q1_KLL — routes to KLL collector via sketch='kll'
    ),  # Q1_KLL
}
...
def resolve_collector_bin(query: str, sketch: str, collector_override: str | None) -> Path:
    ...
    # Q1_KLL always uses the KLL collector regardless of the sketch argument.  # Q1_KLL
    if query == "Q1_KLL":  # Q1_KLL
        return _env_path("COLLECTOR_KLL", DEFAULT_COLLECTOR_PATHS["kll"])  # Q1_KLL
...
def build_plan_body(
    ...
    # Q1_KLL is also covered here: aggregations=('p50','p95','p99') resolves to
    # quantile_grid=[0.0, 0.5, 0.95, 0.99, 1.0] which the KLL collector needs.  # Q1_KLL
    if cfg.sketch_family == "quantile":
        ...
        if quantiles_needed:
            body["quantile_grid"] = quantiles_needed  # Q1_KLL: [0.0, 0.5, 0.95, 0.99, 1.0]
...
def _run_one_query_file(
    ...
    try:
        if not is_nop:
            if query == "Q1_KLL":  # Q1_KLL: bypass controller — use static quantile-mode config
                q1kll_cfg = collector_bin.parent / "config-q1kll.yaml"
                if not q1kll_cfg.is_file():
                    print(f"Q1_KLL config not found: {q1kll_cfg}", file=sys.stderr)
                    return 1
                print(f"Q1_KLL: using static config {q1kll_cfg}", file=sys.stderr)
                with open(results_dir / "collector.log", "wb") as logf:
                    cpid = subprocess.Popen(
                        [str(collector_bin), f"--config={q1kll_cfg}"],
                        stdout=logf,
                        stderr=subprocess.STDOUT,
                        cwd=str(bench_root),
                    )
        ...
        if query == "Q1_KLL":  # Q1_KLL: send deltas, not raw accumulator totals
            inc_cols = ",".join(
                s["column"] for s in Q1_KLL_STREAMS if s.get("input") == "increment"
            )
            if inc_cols:
                replay_argv += ["--increment-cols", inc_cols]
