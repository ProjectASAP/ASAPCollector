...
| **topk / heavy-hitter** | **CountSketch**, **Count-Min (CMS-heap)** | `_topk_cs`, `_topk_cms` | recall@k, by-count vs by-value-sum, wire |
| **frequency** (point count of an item) | **Count-Min**, **CountSketch** | `_freq_cms`, `_freq_cs` | one-sided (CMS) vs unbiased (CS) error |
...
| `gct-topk-host` | `topk(10, sum by (machine) (gct_cpu_rate_topk_<sketch>))` | topk | **CountSketch, CMS-heap** | 2,6 | Fig 3c topk recall (CS vs CMS) |
| `gct-freq-service` | `count_over_time(gct_cpu_rate_freq_<sketch>{service="svc-000003"}[300s])` | frequency | **Count-Min, CountSketch** | 2,6 | one-sided vs unbiased freq |
...
| `debs-topk-active` | `topk(10, sum by (symbol) (debs_ticks_topk_<sketch>))` | topk | **CountSketch, CMS-heap** | 2,7 | most-active-symbol board |
...
| `ms-topk-slow` | `topk(10, sum by (service) (alibaba_ms_latency_topk_<sketch>))` | topk | **CountSketch, CMS-heap** | 2,6 | slowest-service board |
...
| `cx-topk-active` | `topk(10, sum by (pair) (crypto_trades_topk_<sketch>))` | topk | **CountSketch, CMS-heap** | 2,7 | most-active-pair board |
...
|---|---|---|---|---|
| Google cluster 2019 | M1 | quantile {DD,KLL}, topk {CS,CMS}, freq {CMS,CS}, card {HLL}, sum, threshold | rare forensic point | Fig 3 accuracy, Fig 7 latency, Pareto |
| DEBS-2022 | M2 | quantile {DD,KLL}, VWAP {Sum}, topk {CS,CMS}, **coordinated sampling** | audit/backtest replay | **Fig 9** 32× sampling, edge-Gorilla cold |
| Alibaba microservices | M1 | quantile {DD,KLL} @scale, card {HLL}, rate {Count}, topk {CS,CMS} | trace replay (separate span) | high-card disjoint (Fig 11), Fig 7 |
...
| Crypto tick | M2 | quantile {DD,KLL}, VWAP {Sum}, topk {CS,CMS}, **short-circuit** | backtest replay | **bound-based short-circuit** per family |

**Read:** every dataset runs the **same query kinds against the same full sketch sets**
(quantile → DDSketch + KLL, topk → CountSketch + CMS-heap, frequency → Count-Min +
