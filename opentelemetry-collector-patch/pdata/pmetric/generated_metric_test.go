...
func TestMetric_DDSketch(t *testing.T) {
	ms := NewMetric()
	ms.SetEmptyDDSketch()
	assert.Equal(t, NewDDSketch(), ms.DDSketch())
	ms.orig.GetData().(*internal.Metric_DDSketch).DDSketch = internal.GenTestDDSketch()
	assert.Equal(t, MetricTypeDDSketch, ms.Type())
	assert.Equal(t, generateTestDDSketch(), ms.DDSketch())
	sharedState := internal.NewState()
	sharedState.MarkReadOnly()
	assert.Panics(t, func() { newMetric(internal.NewMetric(), sharedState).SetEmptyDDSketch() })
}
