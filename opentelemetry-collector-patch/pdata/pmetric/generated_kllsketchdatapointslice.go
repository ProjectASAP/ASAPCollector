...
// KLLSketchDataPointSlice logically represents a slice of KLLSketchDataPoint.
//
// This is a reference type. If passed by value and callee modifies it, the
// caller will see the modification.
//
// Must use NewKLLSketchDataPointSlice function to create new instances.
// Important: zero-initialized instance is not valid for use.
type KLLSketchDataPointSlice struct {
	orig  *[]*internal.KLLSketchDataPoint
	state *internal.State
}

func newKLLSketchDataPointSlice(orig *[]*internal.KLLSketchDataPoint, state *internal.State) KLLSketchDataPointSlice {
...
func NewKLLSketchDataPointSlice() KLLSketchDataPointSlice {
	...
	return newKLLSketchDataPointSlice(&orig, internal.NewState())
}
...
func (es KLLSketchDataPointSlice) EnsureCapacity(newCap int) {
	...
	newOrig := make([]*internal.KLLSketchDataPoint, len(*es.orig), newCap)
	copy(newOrig, *es.orig)
	*es.orig = newOrig
}

// AppendEmpty will append to the end of the slice an empty KLLSketchDataPoint.
// It returns the newly added KLLSketchDataPoint.
func (es KLLSketchDataPointSlice) AppendEmpty() KLLSketchDataPoint {
	es.state.AssertMutable()
	*es.orig = append(*es.orig, internal.NewKLLSketchDataPoint())
...
func (es KLLSketchDataPointSlice) RemoveIf(f func(KLLSketchDataPoint) bool) {
	es.state.AssertMutable()
	newLen := 0
...
func (es KLLSketchDataPointSlice) CopyTo(dest KLLSketchDataPointSlice) {
	dest.state.AssertMutable()
	if es.orig == dest.orig {
		return
	}
	*dest.orig = internal.CopyKLLSketchDataPointPtrSlice(*dest.orig, *es.orig)
}
