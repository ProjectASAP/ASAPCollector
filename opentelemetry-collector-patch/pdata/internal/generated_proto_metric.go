...
func (m *Metric) GetDDSketch() *DDSketch {
	if v, ok := m.GetData().(*Metric_DDSketch); ok {
		return v.DDSketch
	}
	return nil
}
...
var (
	...
	ProtoPoolMetric_DDSketch = sync.Pool{
		New: func() any {
			return &Metric_DDSketch{}
		},
	}
...
func CopyMetric(dest, src *Metric) *Metric {
	...
	case *Metric_DDSketch:
		var ov *Metric_DDSketch
		if !UseProtoPooling.IsEnabled() {
			ov = &Metric_DDSketch{}
		} else {
			ov = ProtoPoolMetric_DDSketch.Get().(*Metric_DDSketch)
		}
		...
		case "dDSketch", "dd_sketch":
			{
				...
				ov.DDSketch = NewDDSketch()
				ov.DDSketch.UnmarshalJSON(iter)
				orig.Data = ov
			}
...
func (orig *Metric) MarshalProto(buf []byte) int {
	...
	case *Metric_DDSketch:
		if orig.DDSketch != nil {
			l = orig.DDSketch.MarshalProto(buf[:pos])
			pos -= l
			pos = proto.EncodeVarint(buf, pos, uint64(l))
			pos--
			buf[pos] = 0x6a
