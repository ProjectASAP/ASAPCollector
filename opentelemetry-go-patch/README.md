...
## Benchmarking Aggregators for newly added sketches

DDSketch: 
```bash 
cd DataCollector/opentelemetry-go/sdk/metric 
go test -v ./internal/aggregate -run TestDDSketchInsertThroughput -ddsketch.test.throughput.series=2000 -ddsketch.test.throughput.scrapes=1000
```

Prometheus Histogram baseline:
```bash
cd DataCollector/opentelemetry-go/sdk/metric 
go test -v ./internal/aggregate -run TestHistogramInsertThroughput -histogram.test.throughput.series=2000 -ddsketch.test.throughput.scrapes=1000
```

No-operation baseline:
```bash
go test -v ./internal/aggregate  -run TestNoopInsertThroughput -noop.test.throughput.series=2000 -noop.test.throughput.scrapes=1000  -count=1
```