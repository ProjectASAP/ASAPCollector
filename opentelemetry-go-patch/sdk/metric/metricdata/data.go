...
// It will be one of: Gauge, Sum, Histogram, ExponentialHistogram, Summary, or DDSketch.
...
type DDSketch[N int64 | float64] struct {
	// DataPoints are the individual aggregated measurements with unique
	// attributes.
	DataPoints []DDSketchDataPoint[N]
	// Temporality describes if the aggregation is reported as the change from the
	// last report time, or the cumulative changes since a fixed start time.
	Temporality Temporality
}

func (DDSketch[N]) privateAggregation() {}

// DDSketchEncoding identifies the serialization format used by a DDSketch.
...
type DDSketchDataPoint[N int64 | float64] struct {
	// Attributes is the set of key value pairs that uniquely identify the
	// timeseries.
	Attributes attribute.Set
	// SeriesID is the collector-assigned identifier for this series. When non-zero
	...
	// Sketch is the serialized DDSketch payload.
	Sketch []byte
