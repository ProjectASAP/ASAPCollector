...
func equalAggregations(a, b metricdata.Aggregation, cfg config) (reasons []string) {
	...
	case metricdata.DDSketch[int64]:
		r := equalDDSketch(v, b.(metricdata.DDSketch[int64]), cfg)
		if len(r) > 0 {
			reasons = append(reasons, "DDSketch[int64] not equal:")
			reasons = append(reasons, r...)
		}
	case metricdata.DDSketch[float64]:
		r := equalDDSketch(v, b.(metricdata.DDSketch[float64]), cfg)
		if len(r) > 0 {
...
func equalDDSketch[N int64 | float64](a, b metricdata.DDSketch[N], cfg config) (reasons []string) {
	if a.Temporality != b.Temporality {
		reasons = append(reasons, notEqualStr("Temporality", a.Temporality, b.Temporality))
	}
	r := compareDiff(diffSlices(
		a.DataPoints,
		b.DataPoints,
		func(a, b metricdata.DDSketchDataPoint[N]) bool {
			return len(equalDDSketchDataPoint(a, b, cfg)) == 0
		},
	))
	if r != "" {
		reasons = append(reasons, "DDSketch DataPoints not equal:\n"+r)
	}
	return reasons
}

func equalDDSketchDataPoint[N int64 | float64](a, b metricdata.DDSketchDataPoint[N], cfg config) (reasons []string) {
	if !a.Attributes.Equals(&b.Attributes) {
		reasons = append(reasons, notEqualStr(
			"Attributes",
			a.Attributes.Encoded(attribute.DefaultEncoder()),
	...
	if !cfg.ignoreValue {
		...
		if r := equalExtrema(a.Min, b.Min, cfg); len(r) > 0 {
			reasons = append(reasons, "Min not equal:")
			reasons = append(reasons, r...)
		}
		if r := equalExtrema(a.Max, b.Max, cfg); len(r) > 0 {
			reasons = append(reasons, "Max not equal:")
			reasons = append(reasons, r...)
		}
	}
	...
	if !cfg.ignoreExemplars {
		r := compareDiff(diffSlices(
			...
			b.Exemplars,
			func(a, b metricdata.Exemplar[N]) bool {
				return len(equalExemplars(a, b, cfg)) == 0
			},
		))
		if r != "" {
			reasons = append(reasons, "Exemplars not equal:\n"+r)
...
func hasAttributesDDSketchDataPoint[N int64 | float64](
	dp metricdata.DDSketchDataPoint[N],
	attrs ...attribute.KeyValue,
) (reasons []string) {
	for _, attr := range attrs {
		val, ok := dp.Attributes.Value(attr.Key)
...
func hasAttributesDDSketch[N int64 | float64](
	agg metricdata.DDSketch[N],
	attrs ...attribute.KeyValue,
) (reasons []string) {
	for n, dp := range agg.DataPoints {
		reas := hasAttributesDDSketchDataPoint(dp, attrs...)
		if len(reas) > 0 {
			reasons = append(reasons, fmt.Sprintf("ddsketch datapoint %d attributes:\n", n))
			reasons = append(reasons, reas...)
		}
	}
	return reasons
