...
func (m *errMeter) Float64Gauge(string, ...mapi.Float64GaugeOption) (mapi.Float64Gauge, error) {
	return nil, m.err
}

func (m *errMeter) Int64Gauge(string, ...mapi.Int64GaugeOption) (mapi.Int64Gauge, error) {
	return nil, m.err
}
...
func TestInstrumentationExportMetrics(t *testing.T) {
	...
	inst.ExportMetrics(t.Context(), 2048).End(nil)

	got := collect()
	assert.Equal(t, Scope, got.Scope, "unexpected scope")

	names := make(map[string]struct{}, len(got.Metrics))
	...
	for _, name := range []string{
		...
		_, ok := names[name]
		assert.Truef(t, ok, "expected export self-metric %q", name)
	}
}
