...
// It lives in asap-gorilla-go (next to intchunk) so BOTH sides of the wire can
// import it: the edge agent (producer) assembles a Part with WritePart and
// ships the bytes, and the backend gorilla-merger (consumer) validates with
// OpenPart and serves Series() decode-on-read. It is intentionally standalone:
// it does NOT touch the existing fragment ingest -> tsdb path.
//
